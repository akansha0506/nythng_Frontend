...
export default function LoginModal({ onClose }) {
  ...
  const handleVerifyOtp = async (otp) => {
    ...
    try {
      ...
      if (
        guestCart &&
        JSON.parse(guestCart)?.products?.length > 0
      ) {
        await dispatch(
          updateCartOnLogin()
        ).unwrap();
      ...
      // Throw it back to OtpModal so
      // OtpModal can display the error
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-[#E8EEEE] px-6 py-5">
            ...
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="flex h-12 w-12 items-center justify-center rounded-full border border-[#E5EEEE] bg-white shadow-sm transition hover:bg-[#61b9b9] hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
              aria-label="Close login"
            >
              <X size={20} />
            </button>
            ...
            <div className="mt-6 rounded-[28px] border border-[#E5EEEE] bg-white p-6 shadow-sm">
              ...
              <div className="mt-8 flex overflow-hidden rounded-2xl border border-[#D8E8E8] bg-[#FCFEFE]">
                ...
                <input
                  ...
                  placeholder="Enter Mobile Number"
                  className="flex-1 px-5 py-5 text-lg outline-none placeholder:text-[#A6B5B5] disabled:cursor-not-allowed disabled:opacity-60"
                />
              </div>
